    ...
    public IEnumerator FireArrows()
    {
        ...
        for (int i = 0; i < ProjectileCount; i++)
        {
            ...
            if (player.BuffDuration.ContainsKey(BuffType.ArcherDoubleShot) && player.BuffDuration[BuffType.ArcherDoubleShot].IsApply)
            {
                ...
                PoolManager.Instance.Get<RougeProjectile>().Init(spawnPos + distanceY + distanceX, dir, Range, Speed, Damage * 0.8f);
            ...
            {
                PoolManager.Instance.Get<RougeProjectile>().Init(spawnPos, dir, Range, Speed, Damage);
            }
